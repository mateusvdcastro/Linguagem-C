...
TESTE DE ARVORE BINARIA RUBRO NEGRA
...
void escreverArvore(red_black_tree *t, tree_node *x){
    if (x != t->NIL && x != NULL){
        printf("(");
        printf("C%d", x->data);
        escreverArvore(t, x->left);
        escreverArvore(t, x->right);
        printf(")");
    }
    else{
        if(x==t->NIL){
...
int main(){
    ...
    scanf("%d", &numero);
    while(numero != -1){
        if(tree_search(t, numero) == t->NIL){ //Verifica se o numero ja esta na arvore
        insert(t, new_tree_node(numero));
            //escreverArvore(t, t->root);
        }
        scanf("%d", &numero);
    }
