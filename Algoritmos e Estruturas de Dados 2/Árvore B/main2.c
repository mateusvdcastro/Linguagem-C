...
void search(int key){
    int pos, n;
    struct btreeNode *ptr = root;
    while (ptr){
        n = ptr->count;
        pos = searchPos(key, ptr->item, n);

        if (pos < n && key == ptr->item[pos]){
            printf("%d\n", n);
            return;
        }
        ptr = ptr->link[pos];
...
void searching(int item, int *pos, struct btreeNode *myNode) {
  if (!myNode) { // se a árvore estiver vazia
    return;
  }

  if (item < myNode->item[1]) {
    printf("%d", myNode->item[1]);
    *pos = 0;
  } else {
...
int main() {
    ...
    int key;  // chave a ser inserida ou pesquisada
