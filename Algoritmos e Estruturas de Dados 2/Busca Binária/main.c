...
// A complexidade de uma busca binária é de O(logN)
...
int busca_binaria(int vetor[], int ini, int fim, int item, int *rec)
{
  ...
  if (ini > fim)
  { /* Ponto de parada. Item não está no vetor. Se para a esqueda fim = -1 e início = 0, se para a direita inicio = n + 1 e fim = n. Portanto ini > fim é ponto de parada */
    return -1;
  }
